...
RSpec.describe Event, :type => :model do
	...
	it "should create an event given correct inputs" do
		test_time = Time.new(2020, 10, 31, 2, 30, 0, "-08:00")
		...
		expect(err1).to eq Event::SUCCESS
		expect(err2).to eq Event::SUCCESS
	end
	...
	it "should fail given a bad title" do
		...
		expect(err1).to eq Event::ERR_BAD_TITLE
		expect(err2).to eq Event::ERR_BAD_TITLE
	end
	...
	it "should fail given an invalid event type" do
		...
		expect(err1).to eq Event::ERR_BAD_TYPE
		expect(err2).to eq Event::ERR_BAD_TYPE
	end
	...
	it "should fail given bad info" do
		...
		expect(err1).to eq Event::ERR_BAD_INFO
	end

	#test create bad date
	it "should fail given a bad date" do
		bad_time = Time.new(2000, 10, 31, 2, 30, 0, "-08:00")
		err1 = Event.create_event("Google", "Free Donuts", "info-session", "Come to Soda 651 for infinite donuts", test_time)
		expect(err1).to eq Event::ERR_BAD_DATE 
	end
	...
	it "should create unique ids for events" do
		...
		event1 = Event.find_by(title: "Free Donuts")
		event2 = Event.find_by(title: "Free Apples")
	...
	it "should fail given a nonexistant event_id" do
		...
		expect(err1).to eq Event::ERR_NO_SUCH_EVENT
	end
	...
	it "should fail given and invalid field" do
		...
		expect(err1).to eq Event::ERR_BAD_FIELD
	end
	...
	#test get event
	it "should succesffully retrieve and event" do
		test_time = Time.new(2020, 10, 31, 2, 30, 0, "-08:00")
		err0 = Event.create_event("Google", "Free Donuts", "info-session", "Come to Soda 651 for infinite donuts", test_time)
		event_id - Event.last().event_id
